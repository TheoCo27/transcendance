...
export default function GamesPage() {
  ...
  return (
    <main className="mx-auto flex flex-col w-full max-w-7xl flex-1 px-6 py-8 md:px-10 md:py-12">
      {/* <section className="grid gap-8 rounded-[2.75rem] border border-white/10 bg-surface px-6 py-8 text-text shadow-[0_40px_110px_rgba(15,23,42,0.08)] backdrop-blur md:px-10 md:py-10 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <h1 className="mt-6 text-4xl font-semibold text-text-muted md:text-5xl">
            {room.name}
          </h1>
          <p className="mt-4 max-w-3xl text-base leading-8 text-slate-700">
            Cette page garde uniquement les données de room, les joueurs, les
            usernames, le type de jeu et la configuration.
          </p>
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link to={`/rooms/${room.id}`}>
              <SecondaryButton>Retour à la room</SecondaryButton>
            </Link>
            <Link to="/">
        ...
        <aside className="rounded-4xl bg-slate-950 p-6 text-white shadow-[0_24px_80px_rgba(15,23,42,0.24)]">
          <SectionLabel className="text-white/55">Configuration</SectionLabel>
          ...
          <div className="mt-6 space-y-4">
            <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
              ...
              <p className="mt-2 text-sm text-white/70">{room.name}</p>
            </div>
            ...
            <div className="rounded-3xl border border-white/10 bg-white/5 p-4">
              ...
              {roomConfigEntries.length > 0 ? (
                <dl className="mt-3 space-y-2 text-sm text-white/82">
                  {roomConfigEntries.map(([key, value]) => (
                    <div
                      key={key}
      ...
      <section className="mt-8 grid gap-6 lg:grid-cols-[0.9fr_1.1fr] xl:grid-cols-[0.85fr_1.15fr]">
        {/* <div className="rounded-4xl border border-white/10 bg-surface p-6 text-text shadow-[0_24px_70px_rgba(15,23,42,0.07)]">
          <SectionLabel className="text-slate-400">Joueurs</SectionLabel>
          <SectionHeader>Participants</SectionHeader>
          <div className="mt-6 space-y-3">
            {room.players.map((player) => (
              <div
                key={player.userId}
                className="flex items-center justify-between rounded-3xl border border-slate-200 bg-white/70 px-4 py-3"
              >
                <div>
                  <p className="text-sm font-medium text-slate-900">
                    {playerNamesById[player.userId] ??
